      ...
      <div class="layui-tab">
        <blockquote class="layui-elem-quote news_search">
				<form class="layui-form" id="dataForm"
					action="${pwd }/admin/user/findOffinfo" method="post">
					<div class="layui-inline">
						...
						<input class="layui-btn search_btn" type="button" value="发布活动" onclick="locationHref('${pwd }/admin/offinfo/addOff')">
					</div>
				</form>
          ...
          <div class="layui-tab-item layui-field-box layui-show">
            <table class="layui-table table-hover" lay-even="" lay-skin="nob">
              <thead>
                ...
                <c:set var = "i" value = "0"/>
                <c:forEach items="${offinfos}" var="offinfo" varStatus="stat">
					<tr id="data_${stat.index}" align="center" class="main_trbg" onMouseOver="move(this);" onMouseOut="out(this);">
						<td><input type="checkbox" id="box_${stat.index}" value="${offinfo.id}"></td>
						<td name="actid">${offinfo.id}</td>		
						<td name="actid">${offinfo.actid}</td>
						<td name="actid">${temp_users[i].username}</td>
						<td name="userid">${offinfo.name}</td>											
		                  <td name="introduce">${offinfo.introduce }</td>
						<td name="actid">${temp_actTypes[i].actname}</td>
						 ...
						 <td>
	                    <a class="layui-btn layui-btn-mini news_edit" onclick="locationHref('${pwd }/admin/offinfo/updateOffinfo?flag=1&id=${offinfo.id }')"><i class="iconfont icon-edit"></i> 编辑</a>
	                    <a class="layui-btn layui-btn-danger layui-btn-mini news_del" data-id="1"  onclick="delDataByUrl('${pwd }/admin/offinfo/removeOffinfo?id=${offinfo.id}',this)"><i class="layui-icon"></i> 删除</a>
	                  </td>
					</tr>
