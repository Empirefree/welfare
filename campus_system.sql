...
CREATE TABLE if not exists support(
    ...
    distin int not null comment "接受者  区别 0代表个人成功,1官方活动成功,2个人失败，3活动失败，4个人活动成功",
...
CREATE TABLE if not exists actinfo(
    ...
    acttypeid int not null comment "活动类型id",
...
CREATE TABLE if not exists offinfo(
    ...
    introduce text not null comment "活动介绍",
    acttypeid int not null comment "活动类型",
    starttime datetime not null comment "开始时间",
    endtime Datetime not null comment "结束时间",
)engine=InnoDB auto_increment=1 default charset=utf8;
...
INSERT INTO item_type VALUES(null,'现金');
INSERT INTO item_type VALUES(null,'其他');
